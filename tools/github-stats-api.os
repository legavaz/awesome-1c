...
Функция Инициализировать()
    ...
    Репозитории1CПолноценые = Новый Соответствие;
	...
	СтатистикаРепозиториев = Новый Соответствие;
    ...
    Попытка
        Пока ОсталосьОбработать > 0 Цикл
            ...
            Для каждого ЭлементРезультата из СписокРезультатовПоиска Цикл
				...
				СтатистикаРепозиториев[НаименованиеПроекта] = Новый Структура(
                    "Важность, Популярность, Вовлеченность",
					Важность, Популярность, Вовлеченность
                )
...
Процедура ВывестиРезультатыПоискаВMarkdown()
    ...
    ||---|---|---|---|---|
    |" + СоответствиеВMarkdownТаблицу(Репозитории1CПолноценые, Истина) + "
    |
    |
    ...
    || Репозиторий | URL | ВЖН | ППЛ | ЗВЛ |
    ||---|---|---|---|---|
    |" + СоответствиеВMarkdownТаблицу(РепозиторииOS, Истина) + "
    |");
...
Функция СоответствиеВMarkdownТаблицу(_соответствие, ВыводитьСтатистику = Ложь)
    ...
    Для каждого ключЗначение Из отсортированноеСоответствие Цикл
		...
		Если ВыводитьСтатистику Тогда
			...
			строкаСтатистики = 
            	" "    + СтатистикаРепозиториев.Получить(ключЗначение.Ключ).Важность +   
            	" | "  + СтатистикаРепозиториев.Получить(ключЗначение.Ключ).Популярность + 
                " | "  + СтатистикаРепозиториев.Получить(ключЗначение.Ключ).Вовлеченность + " |";

        КонецЕсли;

        СтроковоеПредставлениеСоответствия = СтроковоеПредставлениеСоответствия + "| " + 
        ключЗначение.Ключ + " | [link](" + ключЗначение.Значение + ") |" + строкаСтатистики + "
        |";
